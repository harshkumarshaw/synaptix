...
const INSTITUTION_URL = process.env.NEXT_PUBLIC_INSTITUTION_URL || "http://localhost:8007";
const WORKFLOW_URL = process.env.NEXT_PUBLIC_WORKFLOW_URL || "http://localhost:8010";
...
api.interceptors.request.use((config) => {
  ...
  } else if (url.startsWith("/attendance") || url.startsWith("/events") || url.startsWith("/leave")) {
    config.baseURL = API_URL;
  } else if (url.startsWith("/electives") || url.startsWith("/doap") || url.startsWith("/logbook")) {
    config.baseURL = LOGBOOK_URL;
  } else if (url.startsWith("/institutions") || url.startsWith("/campus") || url.startsWith("/departments")) {
