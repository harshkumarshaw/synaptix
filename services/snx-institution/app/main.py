"""
snx-institution — Synaptix Institution Lifecycle & Profile Service.
"""
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logger.info(
        "snx-institution starting",
        extra={"env": settings.env, "version": "0.1.0"},
    )

    yield

    logger.info("snx-institution shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Synaptix Institution Service",
        description="Institution profile and student lifecycle management service",
        version="0.1.0",
    ...
    async def root() -> dict[str, str]:
        return {
            "message": "Welcome to Synaptix Institution Service",
            "docs": "/docs",
            "health": "/health",
            "status": "running",
        }
    ...
    async def health() -> dict[str, str]:
        return {"status": "ok", "service": "snx-institution", "version": "0.1.0"}

    @app.get("/ready", tags=["infrastructure"])
    async def ready() -> dict[str, str]:
        return {"status": "ready", "service": "snx-institution"}
