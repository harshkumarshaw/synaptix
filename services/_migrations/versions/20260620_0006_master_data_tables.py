...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id", "curriculum_id"], ["curricula.tenant_id", "curricula.id"], ondelete="SET NULL"),
        sa.UniqueConstraint("tenant_id", "id", name="uq_master_data_entities_tenant_id"),
    )
    
    op.create_index("idx_master_data_entities_tenant", "master_data_entities", ["tenant_id"], postgresql_where=sa.text("deleted_at IS NULL"))
    op.create_index(
