...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "tenant_id", "academic_year_id", "code", name="uq_batches_tenant_year_code"
        ),
    )
    op.create_index(
        "idx_batches_tenant",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.UniqueConstraint(
            "tenant_id", "day_of_week", "start_time", "end_time", name="uq_timetable_slots_time"
        ),
    )
    op.create_index(
        "idx_timetable_slots_tenant",
        "timetable_slots",
        ["tenant_id"],
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    ...
    op.create_index(
        "idx_timetable_entries_tenant",
        "timetable_entries",
        ["tenant_id"],
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
    op.create_index(
        "idx_timetable_entries_batch",
        "timetable_entries",
        ["batch_id"],
        postgresql_where=sa.text("deleted_at IS NULL"),
    )
