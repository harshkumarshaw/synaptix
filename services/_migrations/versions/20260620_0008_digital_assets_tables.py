...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("meta_attributes", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default="{}"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "uploaded_by"], ["users.tenant_id", "users.id"], ondelete="RESTRICT"),
        sa.UniqueConstraint("tenant_id", "id", name="uq_digital_assets_tenant_id"),
    )
    op.create_index("idx_digital_assets_tenant", "digital_assets", ["tenant_id"], postgresql_where=sa.text("deleted_at IS NULL"))
    op.create_index("idx_digital_assets_sha256", "digital_assets", ["tenant_id", "sha256"], postgresql_where=sa.text("deleted_at IS NULL"))
