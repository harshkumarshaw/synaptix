...
down_revision: Union[str, None] = '20260620_0009'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id", "signed_off_by"], ["faculty.tenant_id", "faculty.id"], ondelete="RESTRICT"),
        sa.UniqueConstraint("tenant_id", "id", name="uq_foundation_course_records_tenant_id"),

        sa.CheckConstraint("module_name IN ('orientation', 'skills_acquisition', 'professional_development', 'language_computer', 'sports_yoga', 'hospital_visits')", name="chk_foundation_module_name"),
    )
    op.create_index("idx_foundation_student_status", "foundation_course_records", ["tenant_id", "student_id", "is_completed"], postgresql_where=sa.text("deleted_at IS NULL"))
    ...
    op.create_table(
        ...
        sa.CheckConstraint("professional_phase IN ('Phase I', 'Phase II', 'Phase III Part I', 'Phase III Part II')", name="chk_aetcom_professional_phase"),
        sa.CheckConstraint("status IN ('pending', 'reflection_submitted', 'completed')", name="chk_aetcom_status"),
    )
    op.create_index("idx_aetcom_student_phase", "aetcom_records", ["tenant_id", "student_id", "professional_phase"], postgresql_where=sa.text("deleted_at IS NULL"))
