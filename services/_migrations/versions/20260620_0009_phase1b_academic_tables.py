...
down_revision: Union[str, None] = '20260620_0008'
...
def upgrade() -> None:
    ...
    op.add_column(
        "courses",
        sa.Column("default_attendance_category", sa.String(30), nullable=False, server_default="theory")
    )
    op.create_check_constraint(
        "chk_courses_default_attendance_category",
        "courses",
        sa.text("default_attendance_category IN ('theory', 'practical', 'clinical', 'doap', 'ece', 'aetcom', 'foundation_course', 'elective')")
    )
    ...
    op.create_unique_constraint("uq_academic_years_tenant_id", "academic_years", ["tenant_id", "id"])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id", "batch_id"], ["batches.tenant_id", "batches.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "academic_year_id"], ["academic_years.tenant_id", "academic_years.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "parent_event_id"], ["events.tenant_id", "events.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["tenant_id", "cancelled_by"], ["users.tenant_id", "users.id"], ondelete="SET NULL"),
        sa.UniqueConstraint("tenant_id", "id", name="uq_events_tenant_id"),

        sa.CheckConstraint("event_type IN ('lecture', 'practical', 'doap', 'ece', 'clinical_posting', 'foundation_course', 'aetcom', 'examination')", name="chk_events_type"),
        sa.CheckConstraint("attendance_category IN ('theory', 'practical', 'clinical', 'doap', 'ece', 'aetcom', 'foundation_course', 'elective')", name="chk_events_attendance_category"),
        sa.CheckConstraint("professional_phase IN ('Phase I', 'Phase II', 'Phase III Part I', 'Phase III Part II')", name="chk_events_professional_phase"),
        sa.CheckConstraint("status IN ('scheduled', 'conducted', 'cancelled', 'rescheduled')", name="chk_events_status"),
    )
    op.create_index("idx_events_batch_date", "events", ["tenant_id", "batch_id", "date"], postgresql_where=sa.text("deleted_at IS NULL"))
    op.create_index("idx_events_year_phase", "events", ["tenant_id", "academic_year_id", "professional_phase"], postgresql_where=sa.text("deleted_at IS NULL"))
    op.create_index("idx_events_status", "events", ["tenant_id", "status"], postgresql_where=sa.text("deleted_at IS NULL"))
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
        sa.UniqueConstraint("tenant_id", "event_id", "faculty_id", name="uq_event_faculty_composite"),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "event_id"], ["events.tenant_id", "events.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["tenant_id", "faculty_id"], ["faculty.tenant_id", "faculty.id"], ondelete="RESTRICT"),
    )
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
        sa.UniqueConstraint("tenant_id", "event_id", "course_id", name="uq_event_courses_composite"),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "event_id"], ["events.tenant_id", "events.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["tenant_id", "course_id"], ["courses.tenant_id", "courses.id"], ondelete="RESTRICT"),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "course_id"], ["courses.tenant_id", "courses.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "curriculum_id"], ["curricula.tenant_id", "curricula.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "workflow_instance_id"], ["workflow_instances.tenant_id", "workflow_instances.id"], ondelete="SET NULL"),
        sa.UniqueConstraint("tenant_id", "id", name="uq_lesson_plans_tenant_id"),

        sa.CheckConstraint("nmc_competency_level IN ('K', 'KH', 'SH', 'P')", name="chk_lesson_plans_competency_level"),
        sa.CheckConstraint("status IN ('draft', 'pending_approval', 'approved', 'rejected')", name="chk_lesson_plans_status"),
    )
    op.create_index("idx_lesson_plans_course_status", "lesson_plans", ["tenant_id", "course_id", "status"], postgresql_where=sa.text("deleted_at IS NULL"))
    op.create_index("idx_lesson_plans_curriculum_comp", "lesson_plans", ["tenant_id", "curriculum_id", "competency_code"], postgresql_where=sa.text("deleted_at IS NULL"))
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("conducted_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "event_id"], ["events.tenant_id", "events.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "lesson_plan_id"], ["lesson_plans.tenant_id", "lesson_plans.id"], ondelete="RESTRICT"),
        sa.UniqueConstraint("tenant_id", "id", name="uq_sessions_tenant_id"),
    )
    op.create_index("idx_sessions_event", "sessions", ["tenant_id", "event_id"], postgresql_where=sa.text("deleted_at IS NULL"))
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
        sa.UniqueConstraint("tenant_id", "session_id", "faculty_id", name="uq_session_faculty_composite"),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "session_id"], ["sessions.tenant_id", "sessions.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["tenant_id", "faculty_id"], ["faculty.tenant_id", "faculty.id"], ondelete="RESTRICT"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id", "from_curriculum_id"], ["curricula.tenant_id", "curricula.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "to_curriculum_id"], ["curricula.tenant_id", "curricula.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["tenant_id", "approved_by"], ["users.tenant_id", "users.id"], ondelete="RESTRICT"),
    )
