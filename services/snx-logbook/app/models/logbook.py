...
class FoundationCourseRecord(TenantScopedBase):
    ...
    completed_hours: Mapped[float] = mapped_column(
        Numeric(4, 2), default=0.0, server_default="0.0", nullable=False
    )
    required_hours: Mapped[float] = mapped_column(Numeric(4, 2), nullable=False)
    is_completed: Mapped[bool] = mapped_column(
        default=False, server_default="false", nullable=False
    )
    signoff_received_at: Mapped[datetime.datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    signed_off_by: Mapped[uuid.UUID | None] = mapped_column(nullable=True)
