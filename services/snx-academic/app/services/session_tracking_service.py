...
class SessionTrackingService:
    ...
    async def conduct_session(
        self, tenant_id: uuid.UUID, session_in: SessionCreate, actor_id: uuid.UUID | None = None
    ) -> Session:
        # Check event existence
        event_stmt = select(Event).where(
            Event.tenant_id == tenant_id, Event.id == session_in.event_id
        )
        ...
        existing_stmt = select(Session).where(
            Session.tenant_id == tenant_id, Session.event_id == session_in.event_id
        )
        existing_res = await self.db.execute(existing_stmt)
        if existing_res.scalar_one_or_none():
            raise ValidationError(
                f"A session has already been recorded for event {session_in.event_id}."
            )
        ...
        session = Session(
            ...
            conducted_at=session_in.conducted_at or datetime.now(UTC),
        ...
        for fac_item in session_in.conducted_faculty:
            sf = SessionFaculty(
                tenant_id=tenant_id, session_id=session.id, faculty_id=fac_item.faculty_id
            )
            self.db.add(sf)
    ...
    async def calculate_syllabus_coverage(
        ...
        stmt_total_comp = select(func.count(func.distinct(LessonPlan.competency_code))).where(
            LessonPlan.tenant_id == tenant_id,
        ...
        stmt_total_topics = select(func.count(LessonPlan.id)).where(
            LessonPlan.tenant_id == tenant_id,
            LessonPlan.course_id == course_id,
            LessonPlan.curriculum_id == curriculum_id,
            LessonPlan.is_current,
        ...
        topic_coverage = float(conducted_topics) / float(total_topics) if total_topics > 0 else 0.0

        # 3. Hours Coverage (Third Metric)
        # Total planned hours
        stmt_total_hours = select(func.sum(LessonPlan.estimated_hours)).where(
            LessonPlan.tenant_id == tenant_id,
            LessonPlan.course_id == course_id,
            LessonPlan.curriculum_id == curriculum_id,
            LessonPlan.is_current,
        ...
        hours_coverage = conducted_hours / total_hours if total_hours > 0.0 else 0.0
