...
def main() -> int:
    ...
    nmc_keywords = ['attendance', 'logbook', 'crmi', 'aetcom', 'electives', 'next_eligibility',
                    'foundation_course', 'competency']
    nmc_changed = any(any(kw in f.lower() for kw in nmc_keywords) for f in files if f.endswith('.py'))
