...
async def seed() -> None:
    ...
    async with async_session() as session:
        ...
        res = await session.execute(
            text("SELECT id FROM academic_years WHERE tenant_id = :tenant_id AND name = '2026-2027'"),
            {"tenant_id": TENANT_ID}
        )
        ay_id = res.scalar()
        if not ay_id:
        ...
        res = await session.execute(
            text("SELECT id FROM curricula WHERE program_id = :program_id AND version_code = 'CBME-2019'"),
            {"program_id": prog_id}
        )
        ...
        res = await session.execute(
            text("SELECT id FROM curricula WHERE program_id = :program_id AND version_code = 'CBME-2023'"),
            {"program_id": prog_id}
        )
        ...
        for name, code in depts:
            ...
            if not d_id:
                d_id = uuid.uuid4()
                await session.execute(
                    text("INSERT INTO departments (id, tenant_id, name, code) VALUES (:id, :tenant_id, :name, :code)"),
        ...
        for name, code, dept_id, default_category in courses_to_seed:
            ...
            if not c_id:
                ...
                await session.execute(
                    text("UPDATE courses SET default_attendance_category = :category WHERE id = :id"),
        ...
        lesson_plans = [
            (uuid.uuid4(), course_ids["ANAT-101"], curr_2023_id, "AN-1.1-LP1", "Introduction to Anatomical Terminology", "K", True, "draft"),
            (uuid.uuid4(), course_ids["ANAT-101"], curr_2023_id, "AN-1.2-LP1", "Epithelial Tissue Structure", "KH", True, "approved"),
            (uuid.uuid4(), course_ids["ANAT-101"], curr_2019_id, "AN-1.1-LP1", "Anatomical Basics in CBME 2019", "K", True, "approved"),
        ]

        for lp_id, course_id, curriculum_id, code, topic, comp_level, is_core, status in lesson_plans:
            ...
            res = await session.execute(
                ...
                {"tenant_id": TENANT_ID, "course_id": course_id, "curriculum_id": curriculum_id, "code": code}
            )
            if not res.scalar():
