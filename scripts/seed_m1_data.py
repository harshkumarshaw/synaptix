...
async def seed() -> None:
    ...
    async with async_session() as session:
        ...
        res = await session.execute(
            text("SELECT id FROM batches WHERE tenant_id = :tenant_id AND academic_year_id = :ay_id AND code = 'MBBS-2026'"),
        ...
        for name, email, mobile in users_to_seed:
            ...
            await session.execute(
                ...
                {"id": user_id, "tenant_id": TENANT_ID, "email": email, "mobile": mobile, "name": name, "hash": password_hash}
            )
            res = await session.execute(
