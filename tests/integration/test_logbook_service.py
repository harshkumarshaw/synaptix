...
async def test_logbook_service_flows(db_session, tenant_id):
    ...
    await db_session.execute(
        text(
            "INSERT INTO users (id, tenant_id, email, full_name, is_active) VALUES (:id, :t_id, 'fac@jmn.edu', 'Faculty User', true)"
        ),
    ...
    await db_session.execute(
        text(
            "INSERT INTO departments (id, tenant_id, name, code) VALUES (:id, :t_id, 'Physiology Dept', 'PHYS_LOG')"
        ),
    ...
    await db_session.execute(
        text(
            "INSERT INTO programs (id, tenant_id, name, code, type, duration_years) VALUES (:id, :t_id, 'MBBS', 'MBBS-CBME-LOG', 'professional_phase', 5)"
        ),
    ...
    await db_session.execute(
        text(
            "INSERT INTO batches (id, tenant_id, academic_year_id, program_id, name, code) VALUES (:id, :t_id, :ay_id, :prog_id, 'MBBS 2026', 'MBBS2026_L')"
        ),
    ...
    h_log = FoundationCourseHoursLog(
        student_id=student_id, module_name="hospital_visits", hours=5.0
    )
    ...
    h_log_2 = FoundationCourseHoursLog(
        student_id=student_id, module_name="hospital_visits", hours=6.0
    )
    ...
    so_payload = FoundationCourseSignoffPayload(student_id=student_id, module_name="orientation")
    r3 = await logbook_service.signoff_foundation_module(
        tenant_id, so_payload, signed_off_by=faculty_id
    )
    ...
    aet_rec = await logbook_service.submit_reflection(
        tenant_id, student_id, aet_submit, actor_id=student_user_id
    )
    ...
    aet_signed = await logbook_service.signoff_aetcom_competency(
        tenant_id, aet_so, signed_off_by=faculty_id
    )
