...
async def test_step_json_validation():
    ...
    with pytest.raises(ValidationError) as exc:
        WorkflowDefinitionCreate(
            ...
            code="leave_approval",
            steps={
                "start": WorkflowStepItem(name="start", required_role="faculty", next_steps=["dean_review"]),
            }
        )
...
async def test_definition_versioning_and_immutability(db_session, tenant_id):
    ...
    def_v1 = await service.create_definition(
        ...
        WorkflowDefinitionCreate(
            ...
            steps={
                "start": WorkflowStepItem(name="start", required_role="faculty", next_steps=["approved"]),
            }
        )
    )
