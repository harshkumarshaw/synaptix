...
from app.models.logbook import AetcomRecord
from sqlalchemy import text
from packages.shared.errors import DuplicateRecordError
...
async def test_aetcom_uniqueness_and_cardinality(db_session, tenant_id):
    ...
    prog_id = uuid.uuid4()
    await db_session.execute(
        text("INSERT INTO programs (id, tenant_id, name, code, type, duration_years) VALUES (:id, :t_id, 'MBBS', 'MBBS-CBME-AET', 'professional_phase', 5)"),
    ...
    batch_id = uuid.uuid4()
    await db_session.execute(
        text("INSERT INTO batches (id, tenant_id, academic_year_id, program_id, name, code) VALUES (:id, :t_id, :ay_id, :prog_id, 'MBBS 2026', 'MBBS2026_A')"),
    ...
    r1 = await logbook_service.submit_reflection(tenant_id, student_id, sub1, actor_id=student_user_id)
    ...
    r2 = await logbook_service.submit_reflection(tenant_id, student_id, sub2, actor_id=student_user_id)
    ...
    r3 = await logbook_service.submit_reflection(tenant_id, student_id, sub3, actor_id=student_user_id)
    assert r3.id == r1.id
