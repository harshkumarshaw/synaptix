...
async def test_lesson_plan_versioning_and_workflow(db_session, tenant_id):
    ...
    await db_session.execute(
        text("INSERT INTO departments (id, tenant_id, name, code) VALUES (:id, :t_id, 'Anatomy Dept', 'ANAT_LP')"),
        {"id": dept_id, "t_id": tenant_id}
    )
    
    prog_id = uuid.uuid4()
    await db_session.execute(
        text("INSERT INTO programs (id, tenant_id, name, code, type, duration_years) VALUES (:id, :t_id, 'MBBS', 'MBBS-CBME-LP', 'professional_phase', 5)"),
        {"id": prog_id, "t_id": tenant_id}
    )
    
    curr_id = uuid.uuid4()
    await db_session.execute(
        text("INSERT INTO curricula (id, tenant_id, program_id, name, version_code) VALUES (:id, :t_id, :p_id, 'CBME 2023', 'CBME-2023-LP')"),
        {"id": curr_id, "t_id": tenant_id, "p_id": prog_id}
    )
    
    course_id = uuid.uuid4()
    await db_session.execute(
        text("INSERT INTO courses (id, tenant_id, curriculum_id, department_id, name, code, default_attendance_category) VALUES (:id, :t_id, :curr_id, :d_id, 'Anatomy Course', 'ANAT-LP-101', 'theory')"),
    ...
    wfi_res = await db_session.execute(
        text("SELECT id, status, current_step, current_assignee_role FROM workflow_instances WHERE id = :id"),
