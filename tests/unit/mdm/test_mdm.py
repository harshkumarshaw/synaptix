...
@pytest.mark.anyio
async def test_mdm_004_reference_constraint_on_delete(
    test_db_session: AsyncSession, tenant_id: uuid.UUID
) -> None:
    ...
    with pytest.raises(IntegrityError):
        await test_db_session.execute(text("DELETE FROM tenants WHERE id = :id"), {"id": tenant_id})
        await test_db_session.commit()
...
@pytest.mark.anyio
async def test_mdm_005_medical_onboarding_templates(
    test_db_session: AsyncSession, tenant_id: uuid.UUID
) -> None:
...
@pytest.mark.anyio
async def test_mdm_006_nursing_onboarding_templates(
    test_db_session: AsyncSession, tenant_id: uuid.UUID
) -> None:
...
@pytest.mark.anyio
async def test_mdm_007_csv_import_validation(
    test_db_session: AsyncSession, tenant_id: uuid.UUID
) -> None:
    ...
    await test_db_session.execute(
        delete(MasterDataEntity).where(
            MasterDataEntity.tenant_id == tenant_id, MasterDataEntity.category == "csv_import_test"
        )
    )
    await test_db_session.commit()
